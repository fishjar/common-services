...
router.get("/", async (ctx, next) => {
  ...
  ctx.body = await models.Wxuser.findAndCountAll({
    where,
...
router.get("/:id", async (ctx, next) => {
  ctx.body = await models.Wxuser.findById(ctx.params.id)
  await next()
})

router.post("/", async (ctx, next) => {
  ctx.body = await models.Wxuser.create(ctx.request.body)
  await next()
})
...
router.patch("/:id", async (ctx, next) => {
  const obj = await models.Wxuser.findById(ctx.params.id)
  ctx.body = await obj.update(ctx.request.body)
  await next()
})

router.delete("/", async (ctx, next) => {
  ctx.body = await models.Wxuser.destroy({
